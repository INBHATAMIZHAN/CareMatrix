 ...
 */
export const compressImage = async (base64: string, maxWidth = 1280, maxHeight = 1280): Promise<string> => {
...
async function executeWithFallback<T>(
  ...
  const models = [PRIMARY_MODEL, "gemini-flash-latest", SECONDARY_MODEL];
  ...
  for (const model of models) {
    try {
      ...
      // If 503 high demand spike, brief backoff retry
...
export const explainPrescriptionSimple = async (medicines: any[], language: string) => {
  ...
  const prompt = `
    Explain the following prescription in very simple, easy-to-understand language for an everyday patient or elderly person.
    The explanation must be in ${language}.
    
    IMPORTANT FORMATTING RULES:
    - Do NOT write long dense paragraphs.
    - Do NOT use raw markdown asterisks or clutter (avoid **bold** marks).
    - Use clean, short bullet points with emojis for each medicine.
    - Keep sentences short, comforting, and clear.
    
    Structure:
    📋 Summary: 1 short sentence
    💊 Each Medicine:
    • Name and simple purpose (what it cures)
    • When to take (Morning / Afternoon / Night)
    • Food rule (Before food or After food)
    ⚠️ Important Precautions:
    • 2-3 brief safety tips (drink water, do not skip doses)
  ...
  return executeWithFallback(
    ...
    () => {
      // Clean patient-friendly structured instructions
      const lines = medicines.map((m: any, i: number) => {
        ...
        return `💊 ${i + 1}. ${name} (${dosage || 'Standard dose'}):
• Purpose: Helps with your prescribed recovery.
• Schedule: Take ${frequency || 'as directed by doctor'}.
• Meal Rule: ${instructions || 'Take with water after meals'}.`;
      });

      return `📋 Prescription Medication Guide:

${lines.join('\n\n')}

⚠️ Important Precautions:
• Finish the complete course even if you feel better.
• Take with a full glass of water.
• If you experience unusual rash or nausea, contact your doctor.`;
    },
...
export const chatWithAssistant = async (message: string, history: any[], language: string, patientData?: any) => {
  const systemInstruction = `
    You are a friendly, compassionate, and highly clear AI Health Assistant for ClinIQ AI.
    Your main goal is to make medical information and prescriptions extraordinarily simple and convenient for patients to understand.
    Always respond in ${language}.

    CRITICAL PATIENT-CONVENIENT FORMATTING RULES (STRICTLY REQUIRED):
    1. NEVER write dense, long, unbroken paragraphs of text. Patients dislike heavy paragraphs.
    2. DO NOT use raw markdown formatting with excessive asterisks (e.g. avoid **word** clutter).
    3. Always deliver answers in crisp, short, bite-sized bullet points and structured sections.
    4. Use helpful, intuitive emojis for rapid visual scanning:
       • 📋 Quick Answer: 1 simple, reassuring sentence answering the user directly.
       • 💊 Medicine & Purpose: Short bullet per medicine in everyday words.
       • ⏰ Exact Timing: Clean schedule breakdown (☀️ Morning, 🌤️ Afternoon, 🌙 Night).
       • 🍽️ Meal Rules: Before food, after food, with milk, or with water.
       • ⚠️ Important Precautions: What to avoid (e.g. alcohol, driving, empty stomach).
       • ❓ If You Miss a Dose: Clear, calm instructions on what to do.
       • 🚨 When to See Doctor: Clear warning signs.
    5. Keep every single bullet point to 1-2 short lines.
    6. Always use simple, warm, everyday vocabulary. Avoid medical jargon; if a clinical term is needed, immediately explain it simply.
    7. If the patient asks a specific question (e.g., "Can I take with milk?"), answer directly with a bulleted summary first.
  ...
  return executeWithFallback(
    ...
    () => {
      ...
      if (lower.includes("blood pressure") || lower.includes("bp")) {
        return `📋 Quick Answer:
Keeping your blood pressure balanced protects your heart and gives you daily energy.

⏰ Daily Action Plan:
• ☀️ Morning Check: Check and write down your BP at the same time each morning.
• 🍽️ Food Habits: Reduce table salt, pickles, and packaged snacks. Eat fresh fruits and vegetables.
• 🚶 Activity: Walk for 25 to 30 minutes daily at a comfortable, gentle pace.
• 💊 Medicine Timing: Take prescribed BP medicine regularly without skipping days.

⚠️ When to Contact Doctor:
• If your systolic number is over 160 or you feel sudden severe dizziness, headache, or chest pressure.`;
      }
      if (lower.includes("sugar") || lower.includes("diabetes")) {
        return `📋 Quick Answer:
Maintaining steady blood sugar prevents tiredness and protects your kidneys and eyes.

🍽️ Easy Food & Meal Rules:
• 🥗 What to Eat: High-fiber foods like leafy greens, whole wheat, and lentils.
• 🚫 What to Avoid: White sugar, sweet drinks, sodas, and fried snacks.
• ⏰ Meal Schedule: Eat smaller meals at fixed times so your sugar levels don't drop.

💊 Medication & Hydration:
• 💧 Drink 2 to 3 liters of water throughout the day.
• 💊 Take your diabetes tablets exactly at prescribed meal times.

⚠️ Safety Tip:
• Keep a small piece of fruit or candy with you in case you feel sudden shakiness or cold sweating.`;
      }
      if (lower.includes("fever") || lower.includes("headache")) {
        return `📋 Quick Answer:
Fever is your body naturally fighting off an infection. Rest and hydration are key.

💊 Immediate Comfort Steps:
• 💧 Stay Hydrated: Drink plenty of warm water, oral rehydration fluids, or light soups.
• 🛌 Complete Rest: Lie down in a comfortable, breezy room.
• 💊 Medication: Take prescribed Paracetamol strictly after food for pain or temperature.
• ❄️ Cool Compress: Place a damp, cool cloth on your forehead for comfort.

⚠️ When to Call Doctor:
• If fever stays high after 48 hours or you develop a stiff neck or chest cough.`;
      }
      return `📋 Hello! I am your ClinIQ AI Health Assistant.

I am ready to help you with:
• 💊 Explaining your medicines and why you take them.
• ⏰ Daily schedules (Morning ☀️, Afternoon 🌤️, Night 🌙).
• 🍽️ Meal rules (Before meals or After meals).
• ⚠️ Food interactions, side effects, and missed doses.

Tap any of the frequently asked questions below or ask me anytime!`;
    },
    "chatWithAssistant"
  );
};
